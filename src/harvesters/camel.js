...
/**
 * Segmented search queries for Apache Camel routes on GitHub.
 * Each query targets a different Camel pattern.
 */
const SEARCH_QUERIES = [
  { query: '"from(" "to(" camel extension:java', label: 'camel-java-route' },
  { query: '"camel:" "from:" "steps:" extension:yaml', label: 'camel-yaml-route' },
  { query: '"RouteBuilder" "configure" camel extension:java', label: 'camel-route-builder' },
];
...
/**
 * Harvester for Apache Camel route definitions on GitHub.
 * Searches for Java files with RouteBuilder classes and YAML Camel route definitions.
 */
export class CamelHarvester extends BaseHarvester {
  constructor() {
    super(
      'camel',
  ...
  async _harvest(signal) {
    if (!config.github.token) {
      logger.warn('No GITHUB_TOKEN set, skipping Camel harvester');
      return;
    }

    for (const { query, label } of SEARCH_QUERIES) {
      if (signal.aborted) break;
      logger.info(`Camel search: "${query}" [${label}]`);

      try {
        await this._searchAndProcess(query, label, signal);
      } catch (err) {
        if (err.name === 'AbortError') throw err;
        logger.error('Camel search query failed', { query, error: err.message });
      }
  ...
  async _searchAndProcess(query, label, signal) {
    ...
    while (hasMore && page <= MAX_PAGES_PER_QUERY && !signal.aborted) {
      ...
      logger.info(`Camel page ${page}: ${items.length} items (total: ${data.total_count})`);
  ...
  /**
   * Fetch and validate a single file as an Apache Camel route definition.
   */
  async _processFile(searchResult, label, signal) {
    ...
    const validation = this._validateCamel(text, filename);
    if (!validation.valid) {
      this.stats.invalid++;
      return;
    }
    ...
    const normalized = normalizeWorkflow('camel', {
      searchResult,
      content: text,
      label,
      filename,
    ...
    this.stats.new++;
    logger.debug('Stored Camel workflow', {
      id: normalized.id,
  ...
  /**
   * Validate that a file contains an Apache Camel route definition.
   * .java files must contain 'RouteBuilder' or 'from('.
   * .yaml/.yml files must contain 'camel:' and 'from:'.
   * Returns { valid: boolean }.
   */
  _validateCamel(content, filename) {
    const ext = filename.split('.').pop()?.toLowerCase();

    // Java files with Camel route patterns
    if (ext === 'java') {
      const hasRouteBuilder = content.includes('RouteBuilder');
      const hasFrom = content.includes('from(');
      if (hasRouteBuilder || hasFrom) {
        return { valid: true };
      }
    }

    // YAML files with Camel route definitions
    if (['yaml', 'yml'].includes(ext)) {
      const hasCamel = content.includes('camel:');
      const hasFrom = content.includes('from:');
      if (hasCamel && hasFrom) {
        return { valid: true };
      }
