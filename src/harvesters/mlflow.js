...
/**
 * Segmented search queries for MLflow projects on GitHub.
 * Each query targets a different MLflow pattern.
 */
const SEARCH_QUERIES = [
  { query: '"mlflow" "log_metric" extension:py', label: 'mlflow-tracking' },
  { query: '"mlflow.pyfunc" "log_model" extension:py', label: 'mlflow-pyfunc' },
  { query: '"MLproject" "entry_points" filename:MLproject', label: 'mlflow-project' },
];
...
/**
 * Harvester for MLflow project and tracking definitions on GitHub.
 * Searches for Python files with MLflow tracking calls and MLproject files.
 */
export class MLflowHarvester extends BaseHarvester {
  constructor() {
    super(
      'mlflow',
  ...
  async _harvest(signal) {
    if (!config.github.token) {
      logger.warn('No GITHUB_TOKEN set, skipping MLflow harvester');
      return;
    }

    for (const { query, label } of SEARCH_QUERIES) {
      if (signal.aborted) break;
      logger.info(`MLflow search: "${query}" [${label}]`);

      try {
        await this._searchAndProcess(query, label, signal);
      } catch (err) {
        if (err.name === 'AbortError') throw err;
        logger.error('MLflow search query failed', { query, error: err.message });
      }
  ...
  async _searchAndProcess(query, label, signal) {
    ...
    while (hasMore && page <= MAX_PAGES_PER_QUERY && !signal.aborted) {
      ...
      logger.info(`MLflow page ${page}: ${items.length} items (total: ${data.total_count})`);
  ...
  /**
   * Fetch and validate a single file as an MLflow definition.
   */
  async _processFile(searchResult, label, signal) {
    ...
    const validation = this._validateMLflow(text, filename);
    if (!validation.valid) {
      this.stats.invalid++;
      return;
    }
    ...
    const normalized = normalizeWorkflow('mlflow', {
      searchResult,
      content: text,
      label,
      filename,
    ...
    this.stats.new++;
    logger.debug('Stored MLflow workflow', {
      id: normalized.id,
  ...
  /**
   * Validate that a file contains an MLflow definition.
   * .py files must contain 'mlflow.', or MLproject files must contain 'entry_points'.
   * Returns { valid: boolean }.
   */
  _validateMLflow(content, filename) {
    const ext = filename.split('.').pop()?.toLowerCase();
    const baseName = filename.split('/').pop() || filename;

    // Python files with mlflow usage
    if (ext === 'py' && content.includes('mlflow.')) {
      return { valid: true };
    }

    // MLproject files with entry_points
    if (baseName === 'MLproject' && content.includes('entry_points')) {
      return { valid: true };
    }
