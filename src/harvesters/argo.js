...
/**
 * Segmented search queries for Argo Workflows on GitHub.
 * Each query targets a different Argo pattern.
 */
const SEARCH_QUERIES = [
  { query: '"apiVersion: argoproj.io" extension:yaml', label: 'argo-api' },
  { query: '"kind: Workflow" argoproj', label: 'argo-workflow' },
  { query: '"kind: CronWorkflow" extension:yaml', label: 'argo-cron' },
];
...
/**
 * Harvester for Argo Workflow YAML definitions on GitHub.
 * Searches for YAML files containing Argo Workflow and CronWorkflow resources.
 */
export class ArgoHarvester extends BaseHarvester {
  constructor() {
    super(
      'argo',
  ...
  async _harvest(signal) {
    if (!config.github.token) {
      logger.warn('No GITHUB_TOKEN set, skipping Argo harvester');
      return;
    }

    for (const { query, label } of SEARCH_QUERIES) {
      if (signal.aborted) break;
      logger.info(`Argo search: "${query}" [${label}]`);

      try {
        await this._searchAndProcess(query, label, signal);
      } catch (err) {
        if (err.name === 'AbortError') throw err;
        logger.error('Argo search query failed', { query, error: err.message });
      }
  ...
  async _searchAndProcess(query, label, signal) {
    ...
    while (hasMore && page <= MAX_PAGES_PER_QUERY && !signal.aborted) {
      ...
      logger.info(`Argo page ${page}: ${items.length} items (total: ${data.total_count})`);
  ...
  /**
   * Fetch and validate a single file as an Argo Workflow definition.
   */
  async _processFile(searchResult, label, signal) {
    ...
    const validation = this._validateArgo(text, filename);
    if (!validation.valid) {
      this.stats.invalid++;
      return;
    }
    ...
    const normalized = normalizeWorkflow('argo', {
      searchResult,
      content: text,
      label,
      filename,
    ...
    this.stats.new++;
    logger.debug('Stored Argo workflow', {
      id: normalized.id,
  ...
  /**
   * Validate that a file contains an Argo Workflow definition.
   * YAML-like content must contain 'apiVersion' with 'argoproj.io'.
   * Returns { valid: boolean }.
   */
  _validateArgo(content, filename) {
    ...
    const hasArgoproj = content.includes('argoproj.io');

    if (hasApiVersion && hasArgoproj) {
      return { valid: true };
    }
