...
/**
 * Segmented search queries for Home Assistant automations on GitHub.
 * Each query targets a different Home Assistant pattern.
 */
const SEARCH_QUERIES = [
  { query: '"automation:" "trigger:" "action:" extension:yaml', label: 'ha-automation' },
  { query: 'home-assistant "alias:" "trigger:" extension:yaml', label: 'ha-alias' },
  { query: '"blueprint:" "input:" home_assistant extension:yaml', label: 'ha-blueprint' },
];
...
/**
 * Harvester for Home Assistant automation and blueprint definitions on GitHub.
 * Searches for YAML files containing automations, triggers, actions, and blueprints.
 */
export class HomeAssistantHarvester extends BaseHarvester {
  constructor() {
    super(
      'home-assistant',
  ...
  async _harvest(signal) {
    if (!config.github.token) {
      logger.warn('No GITHUB_TOKEN set, skipping Home Assistant harvester');
      return;
    }

    for (const { query, label } of SEARCH_QUERIES) {
      if (signal.aborted) break;
      logger.info(`Home Assistant search: "${query}" [${label}]`);

      try {
        await this._searchAndProcess(query, label, signal);
      } catch (err) {
        if (err.name === 'AbortError') throw err;
        logger.error('Home Assistant search query failed', { query, error: err.message });
      }
  ...
  async _searchAndProcess(query, label, signal) {
    ...
    while (hasMore && page <= MAX_PAGES_PER_QUERY && !signal.aborted) {
      ...
      logger.info(`Home Assistant page ${page}: ${items.length} items (total: ${data.total_count})`);
  ...
  /**
   * Fetch and validate a single file as a Home Assistant definition.
   */
  async _processFile(searchResult, label, signal) {
    ...
    const validation = this._validateHomeAssistant(text, filename);
    if (!validation.valid) {
      this.stats.invalid++;
      return;
    }
    ...
    const normalized = normalizeWorkflow('home-assistant', {
      searchResult,
      content: text,
      label,
      filename,
    ...
    this.stats.new++;
    logger.debug('Stored Home Assistant workflow', {
      id: normalized.id,
  ...
  /**
   * Validate that a file contains a Home Assistant automation or blueprint.
   * Must be .yml/.yaml and contain (automation: or blueprint:) AND (trigger: or action:).
   * Returns { valid: boolean }.
   */
  _validateHomeAssistant(content, filename) {
    ...
    if ((hasAutomation || hasBlueprint) && (hasTrigger || hasAction)) {
      return { valid: true };
    }
