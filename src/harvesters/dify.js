...
/**
 * Segmented search queries for Dify AI app configs on GitHub.
 * Each query targets a different Dify pattern.
 */
const SEARCH_QUERIES = [
  { query: '"app" "model_config" extension:yaml dify', label: 'dify-app' },
  { query: 'filename:*.yaml "prompt_template" dify', label: 'dify-prompt' },
  { query: '"app_mode" "chat" extension:yaml dify', label: 'dify-chat' },
];
...
/**
 * Harvester for Dify AI application YAML configs on GitHub.
 * Searches for YAML files containing Dify app definitions and model configs.
 */
export class DifyHarvester extends BaseHarvester {
  constructor() {
    super(
      'dify',
  ...
  async _harvest(signal) {
    if (!config.github.token) {
      logger.warn('No GITHUB_TOKEN set, skipping Dify harvester');
      return;
    }

    for (const { query, label } of SEARCH_QUERIES) {
      if (signal.aborted) break;
      logger.info(`Dify search: "${query}" [${label}]`);

      try {
        await this._searchAndProcess(query, label, signal);
      } catch (err) {
        if (err.name === 'AbortError') throw err;
        logger.error('Dify search query failed', { query, error: err.message });
      }
  ...
  async _searchAndProcess(query, label, signal) {
    ...
    while (hasMore && page <= MAX_PAGES_PER_QUERY && !signal.aborted) {
      ...
      logger.info(`Dify page ${page}: ${items.length} items (total: ${data.total_count})`);
  ...
  /**
   * Fetch and validate a single file as a Dify app config.
   */
  async _processFile(searchResult, label, signal) {
    ...
    const validation = this._validateDify(text, filename);
    if (!validation.valid) {
      this.stats.invalid++;
      return;
    }
    ...
    const normalized = normalizeWorkflow('dify', {
      searchResult,
      content: text,
      label,
      filename,
    ...
    this.stats.new++;
    logger.debug('Stored Dify config', {
      id: normalized.id,
  ...
  /**
   * Validate that a file contains a Dify app configuration.
   * YAML-like content must contain 'app' and ('model_config' or 'app_mode').
   * Returns { valid: boolean }.
   */
  _validateDify(content, filename) {
    ...
    const hasModelConfig = content.includes('model_config');
    const hasAppMode = content.includes('app_mode');

    if (hasApp && (hasModelConfig || hasAppMode)) {
      return { valid: true };
    }
