...
/**
 * Segmented search queries for Pipedream components on GitHub.
 * Each query targets a different Pipedream pattern.
 */
const SEARCH_QUERIES = [
  { query: '"defineComponent" path:pipedream extension:js', label: 'pipedream-component' },
  { query: '"steps" "trigger" pipedream extension:js', label: 'pipedream-steps' },
  { query: '"defineComponent" "props" extension:mjs pipedream', label: 'pipedream-mjs' },
];
...
/**
 * Harvester for Pipedream component definitions on GitHub.
 * Searches for JS/MJS files containing Pipedream defineComponent and step definitions.
 */
export class PipedreamHarvester extends BaseHarvester {
  constructor() {
    super(
      'pipedream',
  ...
  async _harvest(signal) {
    if (!config.github.token) {
      logger.warn('No GITHUB_TOKEN set, skipping Pipedream harvester');
      return;
    }

    for (const { query, label } of SEARCH_QUERIES) {
      if (signal.aborted) break;
      logger.info(`Pipedream search: "${query}" [${label}]`);

      try {
        await this._searchAndProcess(query, label, signal);
      } catch (err) {
        if (err.name === 'AbortError') throw err;
        logger.error('Pipedream search query failed', { query, error: err.message });
      }
  ...
  async _searchAndProcess(query, label, signal) {
    ...
    while (hasMore && page <= MAX_PAGES_PER_QUERY && !signal.aborted) {
      ...
      logger.info(`Pipedream page ${page}: ${items.length} items (total: ${data.total_count})`);
  ...
  /**
   * Fetch and validate a single file as a Pipedream component.
   */
  async _processFile(searchResult, label, signal) {
    ...
    const validation = this._validatePipedream(text, filename);
    if (!validation.valid) {
      this.stats.invalid++;
      return;
    }
    ...
    const normalized = normalizeWorkflow('pipedream', {
      searchResult,
      content: text,
      label,
      filename,
    ...
    this.stats.new++;
    logger.debug('Stored Pipedream component', {
      id: normalized.id,
  ...
  /**
   * Validate that a file contains Pipedream component code.
   * Must contain 'defineComponent' or both 'steps' and 'trigger'.
   * Returns { valid: boolean }.
   */
  _validatePipedream(content, filename) {
    const ext = filename.split('.').pop()?.toLowerCase();
    if (!['js', 'mjs'].includes(ext)) {
      return { valid: false };
    }

    const hasDefineComponent = content.includes('defineComponent');
    const hasStepsAndTrigger = content.includes('steps') && content.includes('trigger');

    if (hasDefineComponent || hasStepsAndTrigger) {
      return { valid: true };
    }
